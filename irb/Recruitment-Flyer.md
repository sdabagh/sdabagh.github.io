#  FREE STATISTICS COURSE + HELP ADVANCE EDUCATION RESEARCH

---

## Learn Statistics at Your Own Pace with AI-Powered Tutoring Support

**UCLA Dissertation Research Study**

---

##  WHAT YOU GET

###  Complete Introductory Statistics Course
- **12 comprehensive modules** covering all essential statistics topics
- Topics include: data analysis, probability, distributions, hypothesis testing, regression, and more
- Equivalent to a full college-level statistics course

###  Interactive Learning Experience
- Engaging video lessons and real-world examples
- Interactive practice problems with instant feedback
- Quizzes to check your understanding
- Comprehensive study guides and quick reference materials

###  AI Tutor Available 24/7
- Get personalized help whenever you need it
- Ask questions and receive adaptive support
- Hints, guided questions, or step-by-step explanations
- Learn at the level that's right for you

###  Self-Paced Learning
- **No deadlines or due dates!**
- Complete on your own schedule
- Pause and resume anytime
- 100% online - learn anywhere, anytime

###  Certificate of Completion
- Earn a certificate when you finish the course
- Demonstrate your statistics knowledge
- Add to your resume or academic portfolio

###  Help Improve Education for Future Students
- Contribute to cutting-edge research on AI in education
- Your participation helps develop better learning tools
- Support educational equity and access

---

##  WHO CAN PARTICIPATE?

**You are eligible if you:**

-  Are **18 years or older**
-  Are a **community college student** or recent graduate
-  Want to **learn statistics** (no prior experience needed!)
-  Have **internet access** and a computer or tablet

**No statistics background required!** This course starts with the basics and builds your skills step-by-step.

---

##  TIME COMMITMENT

- **Self-Paced**: 4-10 hours per week (you decide!)
- **Duration**: Complete at your own pace (8-16 weeks typical, but go faster or slower as needed)
- **100% Online**: No meetings, no fixed schedule
- **Total Course**: 40-80 hours of learning (equivalent to a 3-unit college course)

---

##  RESEARCH COMPONENT (OPTIONAL)

This free statistics course is part of a UCLA dissertation study investigating how AI tutoring can improve statistics education.

**What does participating in research mean?**
- Your anonymous learning data (quiz scores, time spent, AI interactions) will be analyzed
- You'll complete optional surveys about your experience
- All participation is **voluntary** and **anonymous**
- You can use the platform without participating in research

**Why participate in research?**
- Help advance our understanding of AI in education
- Contribute to developing better learning tools
- Support educational access for future students
- No additional time required beyond using the course

**Your privacy is protected:**
- Completely anonymous - no names or identifying information collected
- All data encrypted and secure
- Only group results published, never individual data

---

##  WHAT YOU'LL LEARN

### Course Topics Include:

**Module 1**: Introduction to Statistics & Data
**Module 2**: Descriptive Statistics
**Module 3**: Probability Fundamentals
**Module 4**: Discrete Probability Distributions
**Module 5**: Normal Distribution
**Module 6**: Sampling Distributions
**Module 7**: Confidence Intervals
**Module 8**: Hypothesis Testing Basics
**Module 9**: Two-Sample Tests
**Module 10**: ANOVA
**Module 11**: Simple Linear Regression
**Module 12**: Chi-Square Tests

**Real-world applications** in business, healthcare, social sciences, and more!

---

##  WHY THIS MATTERS

Statistics skills are valuable in almost every career:
- **Business & Marketing**: Analyze sales data, customer trends
- **Healthcare**: Understand medical research and health data
- **Psychology & Social Sciences**: Conduct and interpret research
- **Technology**: Data analysis and machine learning
- **Education**: Assessment and evaluation
- **Personal Life**: Make informed decisions based on data

Learning statistics opens doors to many opportunities!

---

##  HOW TO GET STARTED

### Interested in Participating?

**1. Visit the Study Website**
 **https://sdabagh.github.io/participate.html**

**2. Read the Informed Consent**
Learn about the study, your rights, and what participation involves

**3. Create Your Account**
Sign up and start learning immediately!

**4. Begin Learning**
Access all 12 modules and the AI tutor right away

### Questions?

 **Email**: dabagh_safaa@smc.edu
 **Visit**: https://sdabagh.github.io/research.html

**Principal Investigator**: Safaa Dabagh
PhD Candidate, UCLA Department of Statistics

---

##  IRB APPROVED

This study has been reviewed and approved by the **UCLA Institutional Review Board (IRB)**.

**IRB Protocol Number**: [To be assigned]
**Questions about participant rights?** Contact UCLA IRB at (310) 825-7122

---

## ⭐ WHAT STUDENTS ARE SAYING

*"The AI tutor was so helpful! It explained things in different ways until I understood."*
— Beta Tester

*"I've always been intimidated by statistics, but this course made it accessible and interesting."*
— Beta Tester

*"Being able to learn at my own pace without deadlines was perfect for my busy schedule."*
— Beta Tester

*"I feel confident I can succeed in a college statistics course now!"*
— Beta Tester

---

##  QUICK FACTS

| Feature | Details |
|---------|---------|
| **Cost** | Completely FREE |
| **Duration** | Self-paced, 8-16 weeks typical |
| **Format** | 100% online |
| **Prerequisites** | None! (Basic algebra helpful) |
| **Support** | AI tutor + email support |
| **Certificate** | Yes, upon completion |
| **Research** | Optional, voluntary |
| **Privacy** | Anonymous and confidential |

---

##  DON'T MISS THIS OPPORTUNITY!

Free, high-quality statistics education + contribute to important research on AI in education!

**Enrollment Opening**: Winter 2026
**Express Interest Now**: https://sdabagh.github.io/participate.html

---

**Study conducted by:**
Safaa Dabagh, PhD Candidate
Department of Statistics
University of California, Los Angeles (UCLA)

**Faculty Advisors:**
Prof. Mark Handcock & Prof. Guang Cheng
UCLA Department of Statistics

**Contact:**
dabagh_safaa@smc.edu
https://sdabagh.github.io

---

*UCLA IRB Approved | Protocol #[TBD] | Anonymous & Confidential*

---

**Scan QR Code to Learn More:**
[QR code placeholder - would link to https://sdabagh.github.io/participate.html]
